 ...
 *   GLOBAL FUNCTIONS
 **********************/
static void clean_dcache_by_addr(const void *addr, size_t size)
{
    uint32_t addr32 = (uint32_t)addr & ~(uint32_t)(31); // Align down to 32 bytes
    uint32_t size32 = size + ((uint32_t)addr - addr32);
    size32 = (size32 + 31) & ~(uint32_t)(31); // Align up to 32 bytes
    SCB_CleanDCache_by_Addr((uint32_t*)addr32, size32);
}
...
static void tft_flush_cb(lv_disp_t * disp, const lv_area_t * area, uint8_t * pxmap)
{
	...
	size_t width = x2_flush - x1_flush + 1;
    size_t height = y2_flush - y1_flush + 1;
    size_t size = width * height * 2; // 2 bytes per pixel for RGB565

    // Clean D-cache for the transmit buffer before DMA
    clean_dcache_by_addr(pxmap, size);
...
static void DMA_TransferComplete(DMA_HandleTypeDef *han)
{
   ...
   if (y_flush_act > y2_flush) {
       ...
       if (lv_disp_flush_is_last(disp)) {
           refr_qry = true; // handled in HAL_DSI_TearingEffectCallback
       } else {
           lv_disp_flush_ready(disp);
       }
#else
       if (lv_disp_flush_is_last(disp)) {
           __DSB();  // Ensure memory operations complete before DSI refresh
